...
def register_user():
        ...
        if len(password) < 8:
                print("Passwords must be 8 characters long.")
                return
        else:
    ...
    try:
        ...
        if game_choice in game_library:
            ...
            if game_info['quantity'] > 0:
                rental_cost = 0 if free else game_info['rental cost']
                if balance >= rental_cost:
                    ...
                    print(f"You do not have enough balance to rent {game_name}")
            else: 
...
#function to top-u user account
...
def redeem_free_rental(username):
    ...
    if points >= 3: 
        ...
        print("\nYou do not have enough points. Need at least 3 points.")
...
def admin_login():
    ...
    if admin == admin_username and adminpass == admin_password: #admin username and password is already encoded above 
...
#function to display games inventory
def display_games_inventory():
    pass
